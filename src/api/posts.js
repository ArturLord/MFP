...
export const fetchPosts = createAsyncThunk(
  ...
  async ({ currentPage, setPhotosTotal, posts }) => {
    ...
    const res = await axios.get(
      `https://deebf77f001a3003.mokky.ru/posts?page=${currentPage}&limit=5`,
    );
...
export const toggleLike = createAsyncThunk(
  ...
  async ({ userId, photoId }, { getState }) => {
    try {
      ...
      await axios.patch(`https://deebf77f001a3003.mokky.ru/posts/${photoId}`, newPhoto);
...
export const sendComment = createAsyncThunk(
  ...
  async ({ nickname, photoId, text, idComment }, { getState }) => {
    try {
      ...
      await axios.patch(`https://deebf77f001a3003.mokky.ru/posts/${photoId}`, newPosts);
